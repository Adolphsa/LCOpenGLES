...
GLuint LCGLTexture::GetTextureID()
{
    return m_texID;
}

GLuint LCGLTexture::CreateGLTextureFromFile(AAssetManager *assetManager, const char* fileName)
{
    m_texID = generateTexture(assetManager, fileName);
    return m_texID;
}

GLuint  LCGLTexture::generateTexture(AAssetManager *assetManager, const char* fileName)
{
...
GLuint  LCGLTexture::createOpenGLTexture(LCImage* pImg)
{
    if (pImg == NULL) {
        return -1;
    }
